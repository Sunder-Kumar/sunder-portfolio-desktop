...
Graduation: ${profile.education.graduation}${
      profile.education.College
        ? `\nCollege: ${profile.education.College.name}\nCollege Degree: ${profile.education.College.Degree}\nCollege Percentage: ${profile.education.College.Percentage}\nCollege Duration: ${profile.education.College.Duration}\nCollege Location: ${profile.education.College.location}`
        : ""
    }`,
...
GitHub: ${profile.github}`,
    ...
    `RESUME:
Sunder's resume is available as a PDF document in the portfolio.
If the visitor asks to see/download the resume, use [OPEN_RESUME].`,
  ].join("\n\n");
}

const systemInstruction = `
You are Ask Sunder Kumar, an intelligent conversational AI assistant for Sunder Kumar's professional portfolio.

Your purpose is to answer visitors naturally, intelligently, and conversationally.

CORE BEHAVIOR:

1. Use the portfolio data supplied with the request as your factual source about Sunder Kumar.
...
- casual wording
- follow-up questions
- conversational references such as "that project", "his internship", "what about his skills?"
...
17. For questions unrelated to Sunder Kumar, briefly answer if appropriate. Otherwise explain that Ask Sunder focuses on Sunder Kumar's professional portfolio.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey || apiKey === "your_gemini_api_key_here") {
      console.error("Ask Sunder: GEMINI_API_KEY is missing");

      return new Response(
        JSON.stringify({
          error: "Ask Sunder is not configured",
        }),
        {
          status: 503,
          headers: {
...
`;
    ...
    console.info("Ask Sunder streaming request", {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          console.info("Ask Sunder stream completed");
        } catch (error) {
          console.error("Ask Sunder streaming error:", error);
    ...
    console.error("Ask Sunder error:", {
    ...
    return new Response(
      JSON.stringify({
        error: "Ask Sunder is temporarily unavailable",
      }),
      {
        status: 503,
        headers: {
