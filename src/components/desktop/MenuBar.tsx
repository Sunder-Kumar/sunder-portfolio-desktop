...
export default function MenuBar({ onOpenSearch }: MenuBarProps) {
  ...
  const handleItemClick = (menu: string, item: string) => {
    ...
    if (menu === 'Sunder' && item === 'Ask Sunder') {
      openApp('askraksha');
    }
    ...
    <div ref={menuRef} className="menu-bar fixed top-0 left-0 right-0 h-7 z-50 flex items-center justify-between px-4 text-sm select-none shadow-sm text-white/90 font-medium">
      <div className="flex items-center space-x-4">
        <div className="relative flex items-center space-x-4">
          ...
          <div
            className="relative"
            onMouseEnter={() => handleMenuEnter('Sunder')}
            onMouseLeave={() => setActiveMenu(null)}
          >
            <button
              className={`px-3 py-1 rounded font-bold transition-colors ${activeMenu === 'Sunder' ? 'bg-white/20' : 'hover:bg-white/10'}`}
              aria-haspopup="menu"
              aria-expanded={activeMenu === 'Sunder'}
              onClick={() => handleMenuClick('Sunder')}
            >
              Sunder
            </button>
            {activeMenu === 'Sunder' && (
              <div className="absolute top-full left-0 mt-0 w-48 py-1 rounded-lg border border-white/10 bg-[#1e1b4b]/90 backdrop-blur-xl shadow-2xl">
                <button
                  ...
                  onClick={() => handleItemClick('Sunder', 'Ask Sunder')}
                >
                  Ask Sunder
                </button>
              </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <button onClick={() => openApp('about')} aria-label="Open About Sunder" className="hover:bg-white/10 p-2 rounded transition-colors"><Info size={14} /></button>
        <div
          className="relative"
